...
from moegoe import commons, utils
from moegoe.mel_processing import spectrogram_torch
from moegoe.models import SynthesizerTrn
from moegoe.text import _clean_text, text_to_sequence
    ...
    while True:
        ...
        if choice == "t":
            ...
            if "[CLEANED]" in text:
                try:
                    stn_tst = get_text(
                        text.replace("[CLEANED]", ""), hps_ms, cleaned=True
                    )
                except:
                    print("Invalid text!")
                    sys.exit(1)
